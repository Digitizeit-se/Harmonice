...
namespace Harmonize.BoundMemoryChannel;


/// <summary>
/// Channel that is Bound to memory size and uses memory as storage.
/// Used by any number of readers and writers concurrently.
/// </summary>
/// <typeparam name="T"></typeparam>
public class BoundMemoryChannel<T> : IChannel<T>
{
    private readonly Channel<T> _channel;
    private bool _isComplete;
    private readonly int _waterMark;
    private bool _waterMarkBlock;
    private readonly int _maxQueueWork;
    public Exception? CompleteError { get; private set; }

    /// <summary>
    /// Create a new instance of <see cref="BoundMemoryChannel{T}"/> 
    /// </summary>
    /// <param name="maxQueueWork">The maximum number of items that may be in the queue at any given time. defaults to int.MaxValue</param>
    /// <param name="waterMark">set low point that need to be reached before the channel will accept posting on the channel.defaults to maxQueueWork / 2 </param>
    public BoundMemoryChannel(int maxQueueWork =  int.MaxValue, int waterMark = 0)
    {
        
        _channel = System.Threading.Channels.Channel.CreateBounded<T>(maxQueueWork);
        _isComplete = false;
        _maxQueueWork = maxQueueWork;
        _waterMark = waterMark != 0  ? waterMark : maxQueueWork / 2;
    }
    ...
    public bool TryWrite(T item)
    {
        ...
        //If we are above the maxQueueWork we will block until we are below the waterMark
        if (count >= _maxQueueWork)
        {
            _waterMarkBlock = true;
            return false;
        }

        //If we are below the waterMark we will not block
        if (!_waterMarkBlock) return _channel.Writer.TryWrite(item);

        //If we are above the waterMark we will block until we are below the waterMark
        if (count > _waterMark) return false;

        //We are below the waterMark so we will remove block and write
        _waterMarkBlock = false;
    ...
    public  ValueTask WriteAsync(T item, CancellationToken token = default)
    {
        try
        {
            return token.IsCancellationRequested ? new ValueTask(Task.FromCanceled<T>(token)) :
                TryWrite(item) ? default : WriteAsyncCore(item, token);
        }
        catch (Exception e)
        {
    ...
    /// <summary>
    /// Wait for space to be available in the channel and then write the item to the channel.
    /// </summary>
    /// <param name="item">item to write to channel.</param>
    /// <param name="token">Optional <see cref="CancellationToken"/>to cancel the operation.</param>
    /// <returns>A <see cref="ValueTask"/> that represents the asynchronous write operation.</returns>
    /// <exception cref="ChannelClosedException"></exception>
    private async ValueTask WriteAsyncCore(T item, CancellationToken token)
    {
        while (await WaitToWriteAsync(token).ConfigureAwait(false))
        {
            if (TryWrite(item))
            {
                return;
    ...
    /// </returns>
    public ValueTask<bool> WaitToWriteAsync(CancellationToken token)
    {
        if (Count() >= _maxQueueWork)
        {
            _waterMarkBlock = true;
        ...
        if (!_waterMarkBlock) return new ValueTask<bool>(true);

        if (Count() > _waterMark) return new ValueTask<bool>(false);
