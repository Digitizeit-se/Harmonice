...
internal class FindFileInApplicationFolder : FileLocationFinder
{
    /// <summary>
    ...
    public FindFileInApplicationFolder(ILogger logger, string? appRoot, FileLocationFinder? nextFinder = null, bool isDebug = false) : base(logger, appRoot, nextFinder, isDebug) { }

    /// <summary>
    /// Check if file exists application root folder, if so set plugin location
    /// </summary>
    ...
    public override void FindFileLogic(string? rootFolder, string assemblyName)
    {
        ...
        var testLocation = Path.GetFullPath(Path.Combine(AppRoot,  $"{assemblyName}.dll")).Replace('\\', Path.DirectorySeparatorChar);
