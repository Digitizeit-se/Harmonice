...
internal class FindFileInPluginNameFolder : FileLocationFinder
{
    /// <summary>
    ...
    public FindFileInPluginNameFolder(ILogger logger, string? appRoot, FileLocationFinder? nextFinder = null, bool isDebug = false) : base(logger, appRoot, nextFinder,isDebug) { }

    /// <summary>
    /// Check if file exists in plugin named folder, if so set plugin location
    /// </summary>
    ...
    public override void FindFileLogic(string? rootFolder, string assemblyName)
    {
        ...
        var testLocation = Path.GetFullPath(Path.Combine(AppRoot, Path.Combine(Path.Combine(rootFolder, assemblyName), $"{assemblyName}.dll"))).Replace('\\', Path.DirectorySeparatorChar);
